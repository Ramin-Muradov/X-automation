...
# Search query representing the three Explore categories
query = 'has:videos ("technology" OR "business" OR "finance" OR "cryptocurrency" OR "crypto" OR "bitcoin" OR "ethereum" OR "tech" OR "market") -is:reply -is:retweet'

print(f"Query: {query}")
print(f"Time range: {start_time}")
...
try:
    ...
    if response.data:
        # Sort them by views in python to see the highest
        processed_tweets = []
        ...
        for idx, (t, user_info, views) in enumerate(processed_tweets[:25]):
            print(f"  [{idx}] by @{user_info['username']} (Followers: {user_info['followers_count']}) | Views: {views} | Likes: {t.public_metrics.get('like_count', 0)} | Text: {t.text[:80]}...")
